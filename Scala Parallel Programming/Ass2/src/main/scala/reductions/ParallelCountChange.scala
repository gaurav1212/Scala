...
object ParallelCountChange {
  ...
  def countChange(money: Int, coins: List[Int]): Int = coins match{
    ...
    case x::Nil => if(money==0) 1 else (if (money>=x) countChange(money-x, coins) else 0)
    case x::xs => if(money>=x) (countChange(money-x, coins) + countChange(money, xs)) else countChange(money, xs)
  }
  ...
  def parCountChange(money: Int, coins: List[Int], threshold: Threshold): Int = {
    if (threshold(money, coins)) {
      ...
      coins match {
        ...
        case x :: xs =>
          if (money >= x) {
            val (c1, c2) = parallel(countChange(money - x, coins), countChange(money, xs))
            c1 + c2
          }
