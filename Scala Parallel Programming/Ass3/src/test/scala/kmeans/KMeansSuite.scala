...
class KMeansSuite extends FunSuite {
  ...
  test("'Kmeans' should work for 'points' == GenSeq((0, 0, 1), (0, 0, -1), (0, 1, 0), (0, 10, 0)) and 'means' == GenSeq((0, -1, 0), (0, 2, 0))") {
    ...
    val eta = 12.25
    val means: GenSeq[Point] = IndexedSeq(mean1, mean2)
    val expectedMean: GenSeq[Point] = IndexedSeq(new Point(0,0,0), new Point(0,5.5,0))
    val calculatedMean: GenSeq[Point] =  KM.kMeans(points, means, eta)
    assert( calculatedMean == expectedMean, s"expectedMean $expectedMean, but got $calculatedMean")
