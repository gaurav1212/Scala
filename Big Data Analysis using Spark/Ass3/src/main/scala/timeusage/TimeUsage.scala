...
object TimeUsage {
  ...
  def dfSchema(columnNames: List[String]): StructType = {
    val field = StructField(columnNames.head, StringType, nullable = false) :: columnNames.tail.map( name => StructField(name, DoubleType, nullable = false))
    StructType(field)
  }
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    val actColNames = Array(
      List("t01", "t03", "t11", "t1801", "t1803"),
      List("t05", "t1805"),
      List("t02", "t04", "t06", "t07", "t08", "t09", "t10", "t12", "t13", "t14", "t15", "t16", "t18")
    )
    def belongs(colName: String, index: Int) : Boolean =  actColNames(index).exists(colName.startsWith(_))

    (columnNames.filter(belongs(_, 0)).map(new Column(_)),
    columnNames.filter(col => belongs(col, 1) && ! belongs(col, 0)).map(new Column(_)),
    columnNames.filter(col => belongs(col, 2) && ! belongs(col, 0) && !belongs( col, 1)).map(new Column(_))
    )
  }
  ...
  def timeUsageSummary(
    ...
    val ageProjection: Column = when($"teage" >= 15.0 && $"teage" <= 22.0, "young").when($"teage">=23.0 && $"teage"<=55.0, "active").otherwise("elder").as("age")
  ...
  def timeUsageGroupedTyped(summed: Dataset[TimeUsageRow]): Dataset[TimeUsageRow] = {
    ...
    def roundOneDec(d:Double) = (d * 10).round / 10d
    ...
    summed
      ...
      case ((working, sex, age), primaryNeeds, work, other) => TimeUsageRow(working, sex, age,  roundOneDec(primaryNeeds), roundOneDec(work), roundOneDec(other))
    }.orderBy('working, 'sex, 'age)
  }
}
